...
class Utils {
  ...
  static scrollToElement(parentEl, scrollDuration){
    const viewportHeight = document.documentElement.clientHeight
    const yPos = getOffset(parentEl).top - (viewportHeight/2)
    this.scrollToY(yPos, scrollDuration)
  }

  static chooseRandomArray(arr){
    const index = Math.floor(Math.random()*arr.length)
    return arr[index] 
  }
  ...
  elementIndexesInView(imgElements){
    const firstLast = []
    const imgElementsArray = Array.from(imgElements)
